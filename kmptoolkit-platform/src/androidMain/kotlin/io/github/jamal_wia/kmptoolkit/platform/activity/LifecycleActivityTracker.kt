 ...
 * by hand from `onResume`/`onPause` — the shape this replaces — is one forgotten override away
 * from pinning a destroyed activity, and the failure is invisible until a heap dump. Registering
 * with the `Application` means every activity in the process is tracked whether or not anyone
 * remembered to wire it up.
 *
 * The reference itself is a [WeakReference] as a second line of defence: even if a lifecycle
 * callback were somehow missed — a process in a strange state, a framework bug, an activity
 * destroyed without pausing — the garbage collector can still reclaim the activity, and
...
internal class LifecycleActivityTracker(
     ...
     *
     * `@Volatile` because it is written from the main thread by the lifecycle callbacks and read
     * by [withActivity] from wherever the caller happens to be.
     */
    @Volatile
    ...
    override fun <R> withActivity(block: (Activity) -> R): R? {
        ...
        if (activity.isFinishing || activity.isDestroyed) {
            // Drop it rather than merely refusing it: a finishing activity is never coming back,
            // and holding even a weak reference to it keeps its (large) object graph reachable
    ...
    /**
     * Clears on pause rather than on stop or destroy. Pause is the earliest point at which this
     * activity is no longer the one the user is interacting with, and the *next* activity's
     * `onResume` has not necessarily run yet — so clearing here is what guarantees the tracker
     * never hands out an activity that is on its way out.
     */
     ...
     * objects, and during a configuration change both exist at once. Clearing on the wrong one
     * would drop the new activity the moment the old one is destroyed.
     */
     ...
     * reference — clear it, and [withActivity] must stop answering.
     */
