...
kmptoolkitPublish {
    pomName.set("KMPToolkit Uploader")
    pomDescription.set(
        "A transactional uploader for shared Kotlin code: enqueue an outgoing effect, and it is " +
            ...
            "implement (UploaderStore), so the module ships with no database dependency at all; " +
...
android {
    namespace = "io.github.jamal_wia.kmptoolkit.uploader"
}
...
kotlin {
    ...
    sourceSets {
        commonMain.dependencies {
            // api, not implementation: Uploader.observe returns a Flow, createUploaderEngine takes a
            ...
            // api, not implementation: createUploaderEngine takes a Logger, so the type is part of
        ...
        androidMain.dependencies {
            ...
            // into a consumer's manifest, and docs/kmptoolkit-uploader/05-platform-notes.md for why
        ...
        // is deliberate beyond the cycle: two independent UploaderStore implementations (this one and
        // InMemoryUploaderStore) cross-check the SPI contract instead of one confirming itself.
    }
}
