...
class UploaderDeliveryTest {
    ...
    fun `an exception thrown by the handler is treated as a retry`() = runTest {
        ...
        assertEquals(UploaderItemState.PENDING, store.find("item-1")?.state)
    }
    ...
    fun `an item written by a newer schema is parked instead of mis-decoded`() = runTest {
        ...
        val engine: UploaderEngine = testEngine(store, listOf(newer), backgroundScope)
        ...
        val downgraded: UploaderEngine = testEngine(store, listOf(older), backgroundScope)
    ...
    fun `an item written by an older schema is still delivered`() = runTest {
        ...
        val engine: UploaderEngine = testEngine(store, listOf(older), backgroundScope)
        engine.enqueue(older, "p")
        val newer = TestHandler(schemaVersion = 3)
        val upgraded: UploaderEngine = testEngine(store, listOf(newer), backgroundScope)
    ...
    fun `a drain pass keeps going after one item parks`() = runTest {
        ...
        val engine: UploaderEngine = testEngine(store, listOf(poison, good), backgroundScope)
