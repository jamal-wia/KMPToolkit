 ...
 *   (the default) derives it from the application id as `<applicationId>.uploader.wake`, so two apps
 ...
 *   WorkManager's floor.
 * @param drainBudget how long [UploaderDrainWorker] keeps the process alive waiting for the queue to
...
public data class WorkManagerWakeConfig(
    ...
    public companion object {
        ...
        public const val WORK_NAME_SUFFIX: String = ".uploader.wake"
    }
}

/** The unique-work name actually used: the configured one, or `<applicationId>.uploader.wake`. */
 ...
 *
 * The work runs [UploaderDrainWorker], which finds the engine through [UploaderEngineRegistry] — so
 * register the engine after starting it, or the wake job has nothing to wait on. See
 * `docs/kmptoolkit-uploader/05-platform-notes.md`.
 *
 ...
 * @return a [WakeScheduler] to hand to [createUploaderEngine].
 */
...
internal class WorkManagerWakeScheduler(
    ...
    override fun scheduleWake() {
        ...
        runCatching {
            val constraints: Constraints = Constraints.Builder()
                ...
                .build()
            val request = OneTimeWorkRequestBuilder<UploaderDrainWorker>()
                ...
                .setInputData(
                    workDataOf(
                        UploaderDrainWorker.DRAIN_BUDGET_MILLIS_KEY to
                            config.drainBudget.inWholeMilliseconds,
                        UploaderDrainWorker.ENGINE_WAIT_MILLIS_KEY to
            ...
            logger.w(failure) { "Could not arm the uploader wake; delivery falls back to app launch." }
        }
    }
    ...
    override fun cancelWake() {
        runCatching { WorkManager.getInstance(context).cancelUniqueWork(workName) }
            .onFailure { failure ->
                logger.d { "Could not cancel the uploader wake: ${failure.message}" }
            }
    }
}
 ...
 * registered engine and calls [UploaderEngine.awaitDrained] — holding WorkManager's execution grant
 ...
 */
public class UploaderDrainWorker(
    context: Context,
    ...
    override suspend fun doWork(): Result {
        val engineWait: Duration = inputData
            ...
            .milliseconds
        val engine: UploaderEngine = UploaderEngineRegistry.await(engineWait) ?: return Result.retry()
    ...
    public companion object {
         ...
         */
        public const val DRAIN_BUDGET_MILLIS_KEY: String = "kmptoolkit_uploader_drain_budget_millis"
