 ...
 *   app's bundle id as `<bundleId>.uploader.drain`. **Whatever it resolves to must also appear in
 ...
 * @param engineWait how long the wake handler waits for [UploaderEngineRegistry] to hold a started
...
public data class BackgroundTaskWakeConfig(
    ...
    public companion object {
        ...
        public const val TASK_IDENTIFIER_SUFFIX: String = ".uploader.drain"
...
/**
 * The identifier actually used: the configured one, or `<bundleId>.uploader.drain`.
 *
 ...
 * `docs/kmptoolkit-uploader/05-platform-notes.md` for the code:
...
public class BackgroundTaskWakeScheduler internal constructor(
    ...
    override fun scheduleWake() {
        ...
        if (!submitted) {
            ...
            armed.value = 0
            logger.w { "BGTaskScheduler refused the uploader wake; delivery falls back to app launch." }
        }
    }
