 ...
 * [OutboxStoreContract][io.github.jamal_wia.kmptoolkit.outbox.testing.OutboxStoreContract] requires
 * of its factory.
 */
internal fun createInMemoryOutboxStorage(): OutboxStorage =
    standaloneStorage(createDriver(outboxDatabaseSchema, name = null))
 ...
 *   rows.
 */
internal fun createFileOutboxStorage(name: String): OutboxStorage =
    standaloneStorage(createDriver(outboxDatabaseSchema, name))
...
/**
 * An [OutboxItem] with defaults, so a check names only the fields it cares about.
 *
 * Deliberately not `OutboxStoreContract.item`: that one belongs to the contract and its defaults are
