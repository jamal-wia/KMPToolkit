...
| `signAndroidReleasePublication` / `signXxxPublication` fails with `Could not read PGP secret key` | The `GPG_KEY` secret is not the raw ASCII-armored key text. `ORG_GRADLE_PROJECT_signingInMemoryKey` must be the exact, unmodified output of `gpg --export-secret-keys --armor <key id>` (the full `-----BEGIN PGP PRIVATE KEY BLOCK-----` … `-----END PGP PRIVATE KEY BLOCK-----` block, pasted as-is) — **not** base64-encoded and not stripped of its header/footer. Re-export with `--armor` and replace the `GPG_KEY` repository secret with that output directly. |
