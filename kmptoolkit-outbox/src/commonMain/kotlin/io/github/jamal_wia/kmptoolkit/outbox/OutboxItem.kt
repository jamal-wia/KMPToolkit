 ...
 *
 * You normally read these — through [Outbox.observe] — rather than construct them. The constructor
 * is public because an [OutboxStore][io.github.jamal_wia.kmptoolkit.outbox.spi.OutboxStore]
 ...
 * @property type the owning handler's [OutboxHandler.type]. Renaming a type orphans every queued
 *   row of that type — they park with "no handler registered".
 * @property payload the encoded payload, produced by [OutboxHandler.encodePayload]. Opaque to the
 *   engine and to the store.
 * @property schemaVersion the [OutboxHandler.schemaVersion] in force when this row was written. A
 ...
 *   [OutboxHandler.orderingKey], or `null` for unordered delivery. Channels are scoped per [type].
 ...
 * @property state where the item sits in its lifecycle. See [OutboxItemState].
 ...
 *   Meaningful only in [OutboxItemState.IN_FLIGHT]; `0` otherwise.
 */
...
/**
 * The persisted lifecycle of an [OutboxItem].
 *
 ...
 */
public enum class OutboxItemState {

    /** Waiting to be attempted, possibly gated by [OutboxItem.nextRunAtEpochMillis]. */
    PENDING,

    /**
     * Delivery was detached to an external executor that will report the outcome later through
     * [OutboxEngine.settle]. The row stays persisted — the effect is still owed until the executor
     * confirms.
     *
     * The accompanying [OutboxItem.leaseUntilEpochMillis] is a **claim, not a lock**: while it is
     ...
     *
     * A parked item never loses data silently. It stays visible through [Outbox.observe], and a
     * fresh enqueue with the same ([OutboxItem.type], [OutboxItem.uniqueKey]) revives the key under
