...
/**
 * The outcome of one [OutboxHandler.execute] attempt, and the only way a handler talks back to the
 * engine.
 *
...
public sealed interface AttemptResult {
     ...
     *
     * The row stays in the queue as [OutboxItemState.PARKED], visible through [Outbox.observe] so
     ...
     *
     * The engine moves the row to [OutboxItemState.IN_FLIGHT] with a persisted lease of
     * [leaseMillis]. The executor must eventually call [OutboxEngine.settle] with the item id it
 ...
 * [OutboxEngine.settle] — the counterpart of [AttemptResult] for the half of the work that happens
 * outside [OutboxHandler.execute].
 *
...
public sealed interface SettleResult {
     ...
     * [OutboxItemState.PENDING] with its lease cleared, under the handler's backoff, and parks or
     * drops if the give-up policy fires.
     *
     * Ignored — with a log line — if the row is no longer [OutboxItemState.IN_FLIGHT], or if its
     * lease has changed since the executor read it. See [OutboxEngine.settle].
     *
