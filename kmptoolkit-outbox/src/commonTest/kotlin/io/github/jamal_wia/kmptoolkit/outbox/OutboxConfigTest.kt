...
/** [OutboxConfig]'s validation and the one knob whose effect is observable from the outside. */
class OutboxConfigTest {
    ...
    fun `a non-positive heartbeat interval is rejected`() {
        assertFailsWith<IllegalArgumentException> { OutboxConfig(heartbeatInterval = Duration.ZERO) }
        assertFailsWith<IllegalArgumentException> { OutboxConfig(heartbeatInterval = (-1).seconds) }
    }

    @Test
    fun `a non-positive minimum alarm delay is rejected`() {
        assertFailsWith<IllegalArgumentException> { OutboxConfig(minAlarmDelay = Duration.ZERO) }
    }

    @Test
    fun `a non-positive drain poll interval is rejected`() {
        assertFailsWith<IllegalArgumentException> { OutboxConfig(drainPollInterval = Duration.ZERO) }
    }

    @Test
    fun `a clock anomaly factor below one is rejected`() {
        assertFailsWith<IllegalArgumentException> { OutboxConfig(clockAnomalyFactor = 0) }
        assertFailsWith<IllegalArgumentException> { OutboxConfig(clockAnomalyFactor = -2) }
    }
    ...
    fun `a larger clock anomaly factor tolerates a further-off gate`() = runTest {
        ...
        fun store(): TestOutboxStore = TestOutboxStore(
            listOf(
                OutboxItem(
                    id = "a",
                    type = handler.type,
        ...
        testEngine(
            ...
            config = OutboxConfig(clockAnomalyFactor = 2),
        ).drain()
        ...
        testEngine(
            ...
            backgroundScope,
            clock = TestClock(millis = 0L),
            config = OutboxConfig(clockAnomalyFactor = 10),
        ).drain()
    ...
    fun `a longer heartbeat interval means fewer safety-net drains`() = runTest {
        ...
        val engine: OutboxEngine = testEngine(
            store,
            listOf(handler),
            backgroundScope,
            config = OutboxConfig(heartbeatInterval = 5.minutes),
        )
        engine.start()
        runCurrent()
        store.insertKeep(
