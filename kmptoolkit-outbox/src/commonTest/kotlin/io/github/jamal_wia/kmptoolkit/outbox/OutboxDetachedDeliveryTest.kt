 ...
 */
class OutboxDetachedDeliveryTest {
    ...
    fun `detaching moves the item to in-flight under a lease without spending retry budget`() =
        runTest {
            ...
            assertEquals(OutboxItemState.IN_FLIGHT, item.state)
    ...
    fun `settling an unknown id is a no-op`() = runTest {
        val store = TestOutboxStore()
        val engine: OutboxEngine = testEngine(store, listOf(TestHandler()), backgroundScope)
    ...
    fun `a failed settle for a parked item does not revive it`() = runTest {
        ...
        val item: OutboxItem = assertNotNull(store.find("item-1"))
        assertEquals(OutboxItemState.PARKED, item.state, "parked is a deliberate decision")
    ...
    fun `a failed settle is rejected when the drain re-hands the item mid-settle`() = runTest {
        ...
        val item: OutboxItem = assertNotNull(store.find("item-1"))
        assertEquals(OutboxItemState.IN_FLIGHT, item.state, "the fresh claim must survive")
    ...
    fun `an item detached by a process that died is picked up by the next one`() = runTest {
        ...
        val first: OutboxEngine = testEngine(store, listOf(dying), backgroundScope, clock = clock)
        first.enqueue(dying, "p")
        first.drain()
        first.close() // the process is gone; the row and its lease are not

        val reviving = TestHandler()
        val second: OutboxEngine = testEngine(store, listOf(reviving), backgroundScope, clock = clock)
