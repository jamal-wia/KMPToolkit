...
/**
 * Ordering channels: strict FIFO within one ([OutboxItem.type], [OutboxItem.orderingKey]) pair, and
...
class OutboxOrderingTest {
    ...
    fun `two handler types sharing a raw ordering key still get separate channels`() = runTest {
        ...
        val engine: OutboxEngine = testEngine(store, listOf(stuck, fine), backgroundScope)
