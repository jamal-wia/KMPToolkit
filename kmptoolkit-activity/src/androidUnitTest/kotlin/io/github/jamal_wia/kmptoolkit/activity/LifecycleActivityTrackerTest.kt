...
class LifecycleActivityTrackerTest {
    ...
    @Test
    fun `an untracked activity does not become the current one`() {
        // The predicate's whole point: a picker or a sign-in screen resuming inside this process
        // must not inherit what the app asked for.
        val tracked: ActivityController<Activity> = launchActivity()
        val filtered = LifecycleActivityTracker(application) { it === tracked.get() }
            .also { it.register() }
        tracked.setup()
    ...
    @Test
    fun `an untracked activity never reaches a listener`() {
        val seen: MutableList<Activity> = mutableListOf()
        val filtered = LifecycleActivityTracker(application) { false }.also { it.register() }
        filtered.addOnActivityResumedListener { activity -> seen += activity }
    ...
    @Test
    fun `pausing an untracked activity does not clear the tracked one`() {
        val tracked: ActivityController<Activity> = launchActivity()
        val filtered = LifecycleActivityTracker(application) { it === tracked.get() }
            .also { it.register() }
        tracked.setup()
        ...
        assertNotNull(filtered.withActivity { activity -> activity })
        filtered.release()
    }
}
