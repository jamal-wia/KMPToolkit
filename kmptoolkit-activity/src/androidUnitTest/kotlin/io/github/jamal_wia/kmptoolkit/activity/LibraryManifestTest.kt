 ...
 *
 * Observing this app's own activity lifecycle needs none — `registerActivityLifecycleCallbacks` is
 * a method on the consumer's own `Application`. There is a permission that sounds adjacent,
 * `REORDER_TASKS`, and it is not: reading which of your activities is resumed is nothing like
 * moving someone else's task. A permission merged from a library manifest appears in every
 * consumer's app silently, and nothing else in the build would notice.
 */
...
class LibraryManifestTest {
    ...
    fun `the library manifest contributes no permission of its own`() {
        ...
        assertEquals(
            ...
            "kmptoolkit-activity needs no permission; anything here leaked into every consumer",
        )
    }
