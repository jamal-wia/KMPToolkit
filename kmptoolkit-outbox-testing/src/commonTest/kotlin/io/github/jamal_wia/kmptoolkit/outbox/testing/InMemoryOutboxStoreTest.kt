...
import io.github.jamal_wia.kmptoolkit.outbox.OutboxEngine
...
import io.github.jamal_wia.kmptoolkit.outbox.createOutboxEngine
...
/** [InMemoryOutboxStore] against the published contract, plus the real engine running on it. */
class InMemoryOutboxStoreTest {

    @Test
    fun `it satisfies every invariant of the OutboxStore contract`() = runTest {
        val checksRun: Int = OutboxStoreContract { InMemoryOutboxStore() }.verifyAll()
    ...
    fun `it starts with the items it was seeded with`() = runTest {
        val store = InMemoryOutboxStore(listOf(item("a"), item("b")))
    ...
    fun `the items flow exposes every state for assertions`() = runTest {
        ...
        assertEquals(listOf(OutboxItemState.PARKED), store.items.value.map { it.state })
    ...
    fun `the real engine retries a failing item against this store`() = runTest {
        ...
        val stored: OutboxItem = assertNotNull(store.snapshot().singleOrNull())
