...
/**
 * [SqlDelightUploaderStoreChecks] against `AndroidSqliteDriver` on a real SQLite, under Robolectric.
 *
...
class AndroidSqlDelightUploaderStoreTest {

    private val checks = SqlDelightUploaderStoreChecks()

    @Test
    fun `it satisfies the UploaderStore contract`() = runTest {
        checks.satisfiesTheUploaderStoreContract()
    }
    ...
    @Test
    fun `a database failure arrives as an UploaderStorageException`() = runTest {
        checks.aDatabaseFailureArrivesAsAnUploaderStorageException()
    }
