...
import io.github.jamal_wia.kmptoolkit.uploader.testing.UploaderStoreContract
 ...
 * [UploaderStoreContract] is neither: a database test needs a driver, a driver needs a platform, and
 ...
 */
internal class SqlDelightUploaderStoreChecks {
    ...
    /**
     * Every invariant `UploaderStore` documents, run against a real SQLite database.
     *
     ...
     */
    suspend fun satisfiesTheUploaderStoreContract() {
        val open: MutableList<UploaderStorage> = mutableListOf()
        try {
            val passed: Int = UploaderStoreContract {
                createInMemoryUploaderStorage().also(open::add).store
            }.verifyAll()
            assertEquals(EXPECTED_CONTRACT_CHECKS, passed)
        } finally {
            open.forEach(UploaderStorage::close)
        }
    }
     ...
     * `UploaderItem.sq`; what this check is for is the outcome the SPI specifies — exactly one write
    ...
    suspend fun concurrentRecordFailuresLetExactlyOneThrough() {
        withStorage { storage ->
            val store: UploaderStore = storage.store
            repeat(CONTENTION_ROUNDS) { round ->
                ...
                val settled: UploaderItem? = store.getById(id)
                assertEquals(UploaderItemState.PENDING, settled?.state)
    ...
    suspend fun aStaleSettleNeverClobbersAFreshClaim() {
        withStorage { storage ->
            ...
            repeat(CONTENTION_ROUNDS) { round ->
                ...
                val row: UploaderItem? = store.getById(id)
                assertEquals(UploaderItemState.IN_FLIGHT, row?.state, "round $round")
    ...
    suspend fun aFailedTransactionRollsBackEveryWriteInIt() {
        withStorage { storage ->
            ...
            // UploaderStorageException is not an IllegalStateException and would never carry this
            // message.
     ...
     *
     * The one invariant `UploaderStoreContract` explicitly says it cannot check, and the one this
     * module exists for: an uploader that loses its rows on restart is a retry queue for the current
     * process only.
     */
    suspend fun aQueueSurvivesCloseAndReopen() {
        val name: String = uniqueDatabaseName("reopen")
        try {
            val original: UploaderItem = item(
                id = "a",
                uniqueKey = "u",
                orderingKey = "o",
                tag = "t",
            ...
            createFileUploaderStorage(name).use { storage ->
    ...
    suspend fun openingAnExistingFileDoesNotRecreateTheSchema() {
        ...
        try {
            createFileUploaderStorage(name).use { storage -> storage.store.insertKeep(item(id = "a")) }
            repeat(3) {
                createFileUploaderStorage(name).use { storage ->
                    assertEquals(listOf("a"), storage.store.getAllActive().map { it.id })
                }
    ...
    suspend fun anUnrecognizedStateReadsBackAsParked() {
        val driver: SqlDriver = createDriver(uploaderDatabaseSchema, name = null)
        val storage: UploaderStorage = createUploaderStorage(driver)
        try {
            val store: UploaderStore = storage.store
            store.insertKeep(item(id = "known"))
            driver.execute(
                identifier = null,
                sql = "UPDATE kmptoolkit_uploader_item SET state = 'FROM_THE_FUTURE' WHERE id = ?",
                parameters = 1,
            ) { bindString(0, "known") }
    ...
    fun closingTwiceIsANoOp() {
        val storage: UploaderStorage = createInMemoryUploaderStorage()
        storage.close()
        storage.close()
    }
     ...
     * silently. That is the failure a transactional uploader exists to prevent, so it has to be the
    ...
    suspend fun closingAStorageDoesNotCloseASuppliedDriver() {
        ...
        try {
            ...
            val rows: Long = driver.executeQuery(
                identifier = null,
                sql = "SELECT count(*) FROM kmptoolkit_uploader_item",
                mapper = { cursor ->
                    cursor.next()
     ...
     * route in full, which is the only one that makes the uploader genuinely transactional.
     */
    ...
    /**
     * A platform database failure arrives as [UploaderStorageException], naming the operation.
     *
     * Not merely tidier: `UploaderStore` has no result type, so a consumer's only handle on a failed
     ...
     */
    suspend fun aDatabaseFailureArrivesAsAnUploaderStorageException() {
        val failure = FakeSqliteException()
        val storage: UploaderStorage = createUploaderStorage(FailingSqlDriver(failure))
        try {
            val store: UploaderStore = storage.store
            val expected: List<Pair<UploaderStorageOperation, suspend () -> Any?>> = listOf(
                UploaderStorageOperation.INSERT_KEEP to { store.insertKeep(item(id = "a")) },
                UploaderStorageOperation.INSERT_REPLACE to { store.insertReplace(item(id = "a")) },
                UploaderStorageOperation.GET_ALL_ACTIVE to { store.getAllActive() },
                UploaderStorageOperation.GET_BY_ID to { store.getById("a") },
                UploaderStorageOperation.RECORD_FAILURE to { store.recordFailure("a", 1, 0L, null) },
                UploaderStorageOperation.MARK_IN_FLIGHT to { store.markInFlight("a", 1L) },
                UploaderStorageOperation.PARK to { store.park("a", null) },
                UploaderStorageOperation.DELETE_BY_ID to { store.deleteById("a") },
                UploaderStorageOperation.DELETE_BY_TAG to { store.deleteByTag("t") },
                UploaderStorageOperation.CLEAR_ALL to { store.clearAll() },
            )
            expected.forEach { (operation, call) ->
                val thrown: UploaderStorageException = assertFailsWith { call() }
                assertEquals(operation, thrown.operation)
                assertEquals(failure, thrown.cause, "the platform failure must be kept as the cause")
            }

            val fromTransaction: UploaderStorageException = assertFailsWith {
                storage.transactionRunner.inTransaction { }
            }
            assertEquals(UploaderStorageOperation.TRANSACTION, fromTransaction.operation)
        } finally {
            storage.close()
        }
    }
    ...
    private suspend fun withStorage(block: suspend (UploaderStorage) -> Unit) {
        val storage: UploaderStorage = createInMemoryUploaderStorage()
        try {
            block(storage)
        } finally {
    ...
    private suspend fun UploaderStorage.use(block: suspend (UploaderStorage) -> Unit) {
        try {
            block(this)
        } finally {
            close()
    ...
    private companion object {
        ...
        /** What [UploaderStoreContract.verifyAll] returns today. */
