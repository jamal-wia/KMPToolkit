...
/**
 * A database operation behind the uploader store failed.
 *
 * ### Why this type exists
 *
 * `UploaderStore` has no result type — every function returns a plain value, so the SPI has no
 ...
 */
public class UploaderStorageException internal constructor(
    public val operation: UploaderStorageOperation,
    override val cause: Throwable?,
) : RuntimeException("uploader storage operation $operation failed", cause)

/**
 * The store function an [UploaderStorageException] came out of. Mirrors
 * [UploaderStore][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore]'s own surface, plus the two
 * entries that are not store functions at all.
 */
public enum class UploaderStorageOperation {

    /** [UploaderStore.insertKeep][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.insertKeep]. */
    INSERT_KEEP,

    /** [UploaderStore.insertReplace][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.insertReplace]. */
    INSERT_REPLACE,

    /** [UploaderStore.getAllActive][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.getAllActive]. */
    GET_ALL_ACTIVE,

    /** [UploaderStore.getById][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.getById]. */
    GET_BY_ID,

    /** [UploaderStore.recordFailure][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.recordFailure]. */
    RECORD_FAILURE,

    /** [UploaderStore.markInFlight][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.markInFlight]. */
    MARK_IN_FLIGHT,

    /** [UploaderStore.park][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.park]. */
    PARK,

    /** [UploaderStore.deleteById][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.deleteById]. */
    DELETE_BY_ID,

    /** [UploaderStore.deleteByTag][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.deleteByTag]. */
    DELETE_BY_TAG,

    /** [UploaderStore.observeByType][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.observeByType]. */
    OBSERVE_BY_TYPE,

    /** [UploaderStore.clearAll][io.github.jamal_wia.kmptoolkit.uploader.spi.UploaderStore.clearAll]. */
    CLEAR_ALL,

    /**
     * [TransactionRunner.inTransaction][io.github.jamal_wia.kmptoolkit.uploader.spi.TransactionRunner.inTransaction]
...
/**
 * Runs [block], turning any platform database failure into an [UploaderStorageException].
 *
 ...
 * quietly keeps going.
 */
internal inline fun <R> mapFailures(operation: UploaderStorageOperation, block: () -> R): R = try {
    block()
} catch (cancellation: CancellationException) {
    throw cancellation
} catch (storage: UploaderStorageException) {
    // Already ours — re-wrapping would bury the operation that actually failed under the outer one.
    throw storage
} catch (failure: Throwable) {
    throw UploaderStorageException(operation, failure)
}
