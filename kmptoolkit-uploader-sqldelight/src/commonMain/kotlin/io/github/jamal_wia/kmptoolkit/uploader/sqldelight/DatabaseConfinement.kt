...
/**
 * The one thread every statement of one [UploaderStorage] runs on.
 *
 ...
 *
 * The cost is one parked thread per [UploaderStorage]. That is why [UploaderStorage.close] exists.
 */
 ...
 *
 * Keyed by [owner] rather than being a bare flag because two [UploaderStorage] instances have two
...
/**
 * The thread one [UploaderStorage] confines its statements to, and the identity nested calls compare
 * against.
 *
...
internal class DatabaseConfinement(val dispatcher: CloseableCoroutineDispatcher) {
     ...
     * caller's code — like a blank name in [UploaderDatabaseConfig] — not a runtime condition an app
     * can recover from.
     */
    fun requireConfined() {
        check(currentThreadId() == threadId) {
            "an uploader storage statement left its database thread — do not switch dispatchers " +
                "inside inTransaction; see docs/kmptoolkit-uploader-sqldelight/03-guide.md"
        }
    }
