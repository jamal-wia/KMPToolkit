 ...
 * `inTransaction { domainWrite(); uploader.enqueue(...) }`, where `enqueue` opens a transaction of its
 ...
 * `docs/kmptoolkit-uploader-sqldelight/03-guide.md` rather than defended against here.
 *
 * ### Failure
 *
    ...
    override suspend fun <R> inTransaction(block: suspend () -> R): R {
        ...
        return withContext(confinement.dispatcher + confinement.marker) {
            ...
            try {
                ...
                throw UploaderStorageException(UploaderStorageOperation.TRANSACTION, machinery)
            }
        }
    }
}
 ...
 * not commit", and would rewrite the caller's exception as an [UploaderStorageException] — hiding
