...
The engine persists through an `OutboxStore` you supply. For a first run, take the in-memory one
from the testing artifact — it is a complete implementation, just not a durable one:

```kotlin
val store: OutboxStore = InMemoryOutboxStore()
```

For production, use `kmptoolkit-outbox-sqldelight`, or write your own against
...
Hand the narrow `Outbox` interface to your features; keep the `OutboxEngine` reference where the
lifecycle lives.

## 5. Enqueue

```kotlin
class MessageRepository(private val outbox: Outbox) {
...
```kotlin
val pending: Flow<List<OutboxItem>> = outbox.observe("chat.send_message")

// e.g. a per-message state for the UI
val stillSending: Flow<Set<String>> = pending.map { items ->
    items.filter { it.state != OutboxItemState.PARKED }
