...
`kmptoolkit-outbox-testing` ships the doubles. Add it as a test dependency:

```kotlin
commonTest.dependencies {
    implementation("io.github.jamal-wia:kmptoolkit-outbox-testing")
}
```

| Fixture | Use it when |
|---|---|
| `InMemoryOutboxStore` | You want the **real engine** in a test — retries, ordering, leases |
| `OutboxStoreContract` | You wrote your own `OutboxStore` and want it held to the contract |
| `FakeOutbox` | You are testing code that only *decides* to enqueue |
...
| `MutableOutboxClock` | You are testing backoff gates or lease expiry |
...
fun `sending a message queues it under its local id`() = runTest {
    ...
    val repository = MessageRepository(outbox)

    repository.send(conversationId = "c1", body = "hi")

    val call = assertNotNull(outbox.lastEnqueued())
...
`FakeOutbox` records the payload **object**, not an encoded string — it never calls your handler's
`encodePayload` — so assertions compare against what you passed in. Simulate a `KEEP` conflict by
passing a result function:

```kotlin
val outbox = FakeOutbox { call -> if (call.uniqueKey == "already-queued") null else "id" }
```

## Testing with the real engine

For anything about delivery, run the engine over `InMemoryOutboxStore`:

```kotlin
@Test
fun `a failing effect is retried with a backoff gate`() = runTest {
    val store = InMemoryOutboxStore()
    val clock = MutableOutboxClock()
    val handler = FailingHandler()
    val engine = createOutboxEngine(
        store = store,
...
If you implemented `OutboxStore`, run the contract against it. One test gets you thirty invariants:
...
influenced by another's leftovers. A violation throws `OutboxStoreContractViolation` naming the
...
Worth reading if you are extending the engine: `kmptoolkit-outbox/src/commonTest` covers retry and
