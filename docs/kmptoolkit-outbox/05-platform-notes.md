...
`OutboxDrainWorker` will not resolve; if that trade matters to you, say so and it can become its own
artifact.
...
OutboxEngineRegistry.register(outbox)
```
...
**What the job does.** Nothing to the queue. `OutboxDrainWorker` waits for the registered engine and
...
**The unique-work name** defaults to `<applicationId>.outbox.wake`. WorkManager's namespace is
...
reflectively by WorkManager, outside your object graph; without `OutboxEngineRegistry.register` it
...
<array>
    <string>com.example.app.outbox.drain</string>
</array>
...
The default identifier is `<bundleId>.outbox.drain`. Read the resolved value from
...
BGTaskScheduler.shared.register(
    forTaskWithIdentifier: OutboxWiring.shared.wakeScheduler.taskIdentifier,
    using: nil
) { task in
    OutboxWiring.shared.wakeScheduler.handleWake { drained in
...
- **Clocks.** Backoff gates and leases are absolute epoch millis, because they must survive process
  death. The engine detects a backwards wall-clock jump (see `OutboxConfig.clockAnomalyFactor`) and
