# kmptoolkit-uploader — overview

A transactional uploader for shared Kotlin code: your app records an outgoing effect **as data**, and
...
An uploader turns "the app owes the server something" into a row. Sending is not a request that either
...
transactionRunner.inTransaction {
    ...
    uploader.enqueue(sendMessageHandler, SendMessage(message), uniqueKey = message.id)
}
```
...
[`UploaderStore`](07-custom-store.md) — with a documented contract, a reference implementation you can
read in one sitting (`InMemoryUploaderStore`), and a runnable contract check
(`UploaderStoreContract`) that holds your implementation to every invariant the engine relies on.
...
If you use SQLDelight, `kmptoolkit-uploader-sqldelight` is a ready store. If you use Room, Realm,
...
| `UploaderStore` | **The SPI you implement** — persistence |
