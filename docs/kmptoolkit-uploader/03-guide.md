...
```kotlin
uploader.enqueue(handler, payload, uniqueKey = "message-$localId")           // KEEP (default)
uploader.enqueue(handler, payload, uniqueKey = "draft-$docId",
...
transactionRunner.inTransaction {
    ...
    uploader.enqueue(sendMessageHandler, SendMessage(message))
}
```
...
```kotlin
val engine: UploaderEngine = UploaderEngineRegistry.await(5.seconds) ?: return Result.retry()
...
```kotlin
uploader.trigger()                       // cheap, non-suspending, conflated — safe from a hot path
uploader.awaitDrained(60.seconds)        // suspend until the queue is empty, for a wake job
```
