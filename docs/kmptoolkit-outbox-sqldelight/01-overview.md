# kmptoolkit-outbox-sqldelight — overview

The storage half of [`kmptoolkit-outbox`](../kmptoolkit-outbox/01-overview.md), on SQLite via
SQLDelight.

`kmptoolkit-outbox` ships with **no database dependency at all**: persistence is an SPI
(`OutboxStore` plus `TransactionRunner`) that you implement against whatever your app already
...
[`07-custom-store.md`](../kmptoolkit-outbox/07-custom-store.md) walks you through — and, more to the
point, the three parts of that page that are easy to get subtly wrong are already right here, and
proved right by `OutboxStoreContract` running against a real database on both platforms:
...
- **`OutboxStore` and `TransactionRunner`**, as a pair, from one factory.
- **Two ways to open it.** Standalone, on a database file of its own, named after your app so two
  ...
  outbox genuinely transactional, because the queue rows and your domain rows are then in the same
  database.
- **A published schema.** `outboxDatabaseSchema` is what you create and migrate when the queue
  lives in your database. Its table is `kmptoolkit_outbox_item` and every index carries the same
...
- **Typed failures.** A SQLite exception never crosses the SPI: it arrives as
  `OutboxStorageException` naming the operation that failed.
...
- **Not a way to avoid `kmptoolkit-outbox`.** It implements that module's SPI and does nothing on
...
- **Not shareable between two processes.** One `OutboxStorage` per queue per process. SQLite would
...
| What the engine does with all this | [`kmptoolkit-outbox/01-overview.md`](../kmptoolkit-outbox/01-overview.md) |
| Implementing a store yourself instead | [`kmptoolkit-outbox/07-custom-store.md`](../kmptoolkit-outbox/07-custom-store.md) |
