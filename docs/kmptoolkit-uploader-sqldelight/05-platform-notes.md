# kmptoolkit-uploader-sqldelight — Platform notes
...
|---|---|---|
| `WAKE_LOCK` | `androidx.work`, via `kmptoolkit-uploader` | Holds the CPU while a queued job runs |
...
They arrive because this module depends on `kmptoolkit-uploader`, whose Android wake layer is
WorkManager. `kmptoolkit-uploader`'s own
[platform notes](../kmptoolkit-uploader/05-platform-notes.md) explain them in full. Both modules'
...
The standalone factories derive the file name from `UploaderDatabaseConfig.name`, which defaults to
your application id. Given a name `N`, the file is `N.kmptoolkit.uploader.db`:

| | Location |
|---|---|
| Android | `/data/data/<your.package>/databases/N.kmptoolkit.uploader.db` — pull it with `adb shell run-as <your.package> cat databases/...` on a debuggable build |
...
**Every statement of one `UploaderStorage` runs on one dedicated thread**, created with the storage
