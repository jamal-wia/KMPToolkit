# kmptoolkit-uploader-sqldelight — guide
...
```kotlin
val storage: UploaderStorage = createUploaderStorage(context)   // Android
val storage: UploaderStorage = createUploaderStorage()          // iOS
```
...
"transactional uploader" actually means, and it is the reason `kmptoolkit-uploader` made storage a port
...
One table and four indices, all prefixed `kmptoolkit_uploader_` so nothing can collide with a table of
yours:

```sql
CREATE TABLE kmptoolkit_uploader_item (
...
CREATE UNIQUE INDEX kmptoolkit_uploader_item_identity
    ON kmptoolkit_uploader_item(type, unique_key) WHERE unique_key IS NOT NULL;
CREATE INDEX kmptoolkit_uploader_item_state ON kmptoolkit_uploader_item(state, sequence);
CREATE INDEX kmptoolkit_uploader_item_type  ON kmptoolkit_uploader_item(type, sequence);
CREATE INDEX kmptoolkit_uploader_item_tag   ON kmptoolkit_uploader_item(tag);
```
...
`kmptoolkit-uploader-sqldelight/src/commonMain/sqldelight/databases/1.db` and verified on every build,
...
YourDatabase.Schema.migrate(
    ...
    oldVersion = old,
    newVersion = new,
    AfterVersion(7) { uploaderDatabaseSchema.create(it) },
)
```
...
1. Bump *your* database version and run `uploaderDatabaseSchema.migrate(driver, from, to)` from that
...
transaction, write a domain row, and call `uploader.enqueue(...)`, which opens one of its own several
frames down.
...
thread that opened them, so this module pins a thread per `UploaderStorage` rather than borrowing a
...
storage.transactionRunner.inTransaction {
    withContext(Dispatchers.IO) { uploader.enqueue(...) }
}
...
silent success there is the exact bug a transactional uploader exists to prevent, so it is the one
...
transaction — or, better, put the slow work in a handler, which is what the uploader is for.
...
cannot be opened — arrives as `UploaderStorageException`, carrying the `UploaderStorageOperation` that
was running and the platform cause.

It is thrown rather than returned because `UploaderStore` has no result type, and swallowing a failed
...
Two `UploaderStorage` instances over the **same** file, on the other hand, is not something to do. The
...
| Concern | Owner |
|---|---|
| Which item runs next, backoff, give-up, lease expiry, ordering | `kmptoolkit-uploader`'s engine |
...
[`kmptoolkit-uploader/03-guide.md`](../kmptoolkit-uploader/03-guide.md). This module has no policy at
all.
