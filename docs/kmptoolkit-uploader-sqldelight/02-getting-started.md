# kmptoolkit-uploader-sqldelight — getting started

A durable queue, running, in about five minutes. Read
[`kmptoolkit-uploader/02-getting-started.md`](../kmptoolkit-uploader/02-getting-started.md) alongside
...
kotlin {
    sourceSets {
        commonMain.dependencies {
            ...
            implementation("io.github.jamal-wia:kmptoolkit-uploader-sqldelight")
        }
    }
}
```
...
`UploaderStoreContract`, `InMemoryUploaderStore`, `FakeUploader` — are all in
`kmptoolkit-uploader-testing` and are storage-agnostic by design, so there is nothing SQLDelight-
specific left to ship. See [`kmptoolkit-uploader/06-testing.md`](../kmptoolkit-uploader/06-testing.md).
...
That is the whole integration. From here everything is `kmptoolkit-uploader`: register a handler,
